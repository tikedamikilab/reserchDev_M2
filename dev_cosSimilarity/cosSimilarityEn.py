...
# DATASET_FILE_PATH = "./datasets_problemText/problemTextA_custom.csv"
...
with open("./datasets_output_custom/tfidf_.csv", 'a',newline='',encoding="utf-8") as f:
    writer = csv.writer(f)
    # tf-idf書き出し用 
    writer.writerow(tf_idf_vectorizer.get_feature_names())
    writer.writerows(tf_idf_array)

with open("./datasets_output_custom/cos_.csv", 'a',newline='',encoding="utf-8") as f:
    writer = csv.writer(f)
    # cos類似度書き出し用
    writer.writerows(cs)
